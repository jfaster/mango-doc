...
由于mango不依赖其它jar包，所以可以直接 `下载mango-1.05.jar <http://search.maven.org/remotecontent?filepath=cc/concurrent/mango/1.05/mango-1.05.jar>`_ ，并将它放在工程的classpath下。

当然mango也已经上传到 `maven中心库 <http://search.maven.org/#artifactdetails%7Ccc.concurrent%7Cmango%7C1.05%7Cjar>`_ 中，如果您的工程在使用maven，那么只需要在pom.xml文件中添加下面的依赖就能使用mango的功能。
